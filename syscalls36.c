...
void	tr_rt_sigtimedwait(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
  ...
  if (the_mode == PARAM)
    {
      ...
      sym->id = symbol_id++;
      sym->symbol_name = "rt_sigtimedwait";
...
void	tr_rt_sigqueueinfo(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
  ...
  if (the_mode == PARAM)
    {
      sym = malloc(sizeof(*sym));
      sym->id = symbol_id++;
      sym->symbol_name = "rt_sigqueueinfo";
      sym->symbol_addr = 0x00000000;
      sym->symbol_type = SYSCALLS;
      sym->debug_info = malloc(MAX_SYSCALLS_CHAR * sizeof(*sym->debug_info));
...
void	tr_rt_sigsuspend(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
...
void	tr_pread64(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
...
void	tr_pwrite64(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
