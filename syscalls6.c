...
void	tr_stime(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
...
void	tr_ptrace(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
  ...
  if (the_mode == PARAM)
    {
      sym = malloc(sizeof(*sym));
      sym->id = symbol_id++;
      sym->symbol_name = "ptrace";
      sym->symbol_addr = 0x00000000;
      sym->symbol_type = SYSCALLS;
      sym->debug_info = malloc(MAX_SYSCALLS_CHAR * sizeof(*sym->debug_info));
      nbChars = sprintf(sym->debug_info, "Parameters (%s, %i, %p, %p",
        p_ptrace_req(reg->ebx, pid),
        (int)reg->ecx,
        (void *)reg->edx,
        (void *)reg->esi);
      sym->sub_symbols = EMPTY;
...
void	tr_alarm(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
  ...
  if (the_mode == PARAM)
  ...
  else
    {
      sym = list_back(graph->sub_symbols);
      nbChars += sprintf(&(sym->debug_info[nbChars]), ")\nReturn");
      sprintf(&(sym->debug_info[nbChars]), " (%u)", (unsigned int)reg->eax);
    }
}

void	tr_oldfstat(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
...
void	tr_pause(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
