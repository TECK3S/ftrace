...
void	tr_sync_file_range(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
  ...
  if (the_mode == PARAM)
    {
      ...
      sym->id = symbol_id++;
      sym->symbol_name = "sync_file_range";
      ...
      nbChars = sprintf(sym->debug_info, "Parameters (%i, %i, %i, %s",
        ...
        (int)reg->ecx,
        (int)reg->edx,
        p_sunc_file_range_flag(reg->esi, pid));
...
void	tr_tee(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
  ...
  if (the_mode == PARAM)
    {
      ...
      sym->symbol_type = SYSCALLS;
      sym->debug_info = malloc(MAX_SYSCALLS_CHAR * sizeof(*sym->debug_info));
      nbChars = sprintf(sym->debug_info, "Parameters (%i, %i, %u, %u, %s",
        (int)reg->ebx,
        (int)reg->ecx,
        (size_t)reg->edx,
        (size_t)reg->esi,
        p_tee_flag(reg->edi, pid));
      sym->sub_symbols = EMPTY;
      list_push_back(&graph->sub_symbols, sym);
    }
  else
...
void	tr_vmsplice(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
  ...
  if (the_mode == PARAM)
    {
      sym = malloc(sizeof(*sym));
      sym->id = symbol_id++;
      sym->symbol_name = "vmsplice";
      sym->symbol_addr = 0x00000000;
      sym->symbol_type = SYSCALLS;
      sym->debug_info = malloc(MAX_SYSCALLS_CHAR * sizeof(*sym->debug_info));
      nbChars = sprintf(sym->debug_info, "Parameters (%i, %s, %lu, %s",
        (int)reg->ebx,
        p_struct_array_iovec(reg->ecx, pid),
        (unsigned long)reg->edx,
        p_vmsplice_flag(reg->esi, pid));
      sym->sub_symbols = EMPTY;
      list_push_back(&graph->sub_symbols, sym);
    }
  else
...
void	tr_move_pages(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
  ...
  if (the_mode == PARAM)
    {
      sym = malloc(sizeof(*sym));
      sym->id = symbol_id++;
      sym->symbol_name = "move_pages";
      sym->symbol_addr = 0x00000000;
      sym->symbol_type = SYSCALLS;
      sym->debug_info = malloc(MAX_SYSCALLS_CHAR * sizeof(*sym->debug_info));
      nbChars = sprintf(sym->debug_info, "Parameters (%i, %lu, %p, %p, %p, %s",
        (int)reg->ebx,
        (unsigned long)reg->ecx,
        (void *)reg->edx,
        (void *)reg->esi,
        (void *)reg->edi,
        p_move_pages_flag(reg->ebp, pid));
      sym->sub_symbols = EMPTY;
      list_push_back(&graph->sub_symbols, sym);
    }
  else
...
void	tr_getcpu(struct user_regs_struct * reg,
      enum mode the_mode,
      pid_t pid,
      t_symbol_graph * graph)
{
